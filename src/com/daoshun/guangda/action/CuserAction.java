		...
		for (CuserInfo coach : cuserlist) {
			...
			for (int i = 0; i < data.length; i++) {
				...
				if (data[i].equals("0")) {
					...
					if (!CommonUtils.isEmptyString(coach.getDrive_school())) {
						coachexcel.setDrive_school(coach.getDrive_school());
					} else if (coach.getDrive_schoolid() != null && coach.getDrive_schoolid() > 0) {
						DriveSchoolInfo school = cuserService.getDriveSchoolInfoByid(coach.getDrive_schoolid());
						if (school != null) {
							coachexcel.setDrive_school(school.getName());
						}
					}
