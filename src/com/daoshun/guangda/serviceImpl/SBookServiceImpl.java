...
public class SBookServiceImpl extends BaseServiceImpl implements ISBookService {
    ...
    @Override
    public List<CuserInfo> getNearByCoach(String pointcenter, String radius, String condition1, String condition2, String condition3, String condition4, String condition5, String condition6, String condition8, String condition9, String condition10, String condition11) {
        ...
        List<CaddAddressInfo> addresslist = (List<CaddAddressInfo>) dataDao.getObjectsViaParam(cuserhql.toString(), params, CommonUtils.parseDouble(longitude, 0), CommonUtils.parseDouble(latitude, 0), CommonUtils.parseDouble(radius, 0));
        // 当前时间 <2015-04-25>
        ...
        if (addresslist != null && addresslist.size() > 0) {
            ...
            hqlCoach.append("from CuserInfo u where coachid in (:cids) and state = 2 and id_cardexptime > :now and coach_cardexptime > :now" + " and drive_cardexptime > :now and car_cardexptime > :now and money >= gmoney and isquit = 0");

            // 真实姓名和教练所属驾校
            if (!CommonUtils.isEmptyString(condition1)) {
                hqlCoach.append(" and (realname like '%" + condition1 + "%' or drive_school like '%" + condition1 + "%' or drive_schoolid in (select schoolid from DriveSchoolInfo where name like  '%" + condition1 + "%')) ");
            }
            // 星级
    ...
    @Override
    public HashMap<String, Object> getCoachList(String condition1, String condition2, String condition3, String condition4, String condition5, String condition6, String condition8, String condition9, String condition10, String condition11, String pagenum) {
        ...
        // 真实姓名和教练所属驾校
        if (!CommonUtils.isEmptyString(condition1)) {
            cuserhql.append(" and (realname like '%" + condition1 + "%' or drive_school like '%" + condition1 + "%' or drive_schoolid in (select schoolid from t_drive_school_info where name like  '%" + condition1 + "%')) ");
        }
        // 星级
        if (!CommonUtils.isEmptyString(condition2)) {
