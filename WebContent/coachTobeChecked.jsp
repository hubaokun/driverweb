...
<body onload="getTop()" style="min-width: 1850px">
	<div id="content">
		...
		<div id="content_form">
			<div id="content_form_top" style="display:none" >
					...
					<div class="serchcontentdiv" style="float: left; margin-left: 50px; width: 156px" >
						...
						<input id="phone" type="text" class="searchdiv" style="width:100px;font-family: 微软雅黑;" value="${searchphone}" onchange="coachPhoneidNum()"/>
					</div>
...
<!-- 			<div id="content_form_top" style="disiplay:none" >
			...
			<div class="addbutton" style="margin-left: 35px;background: #4cc2ff" onclick="allpass()">审核通过</div>
			                          ...
			                          <div class="addbutton" style="background: #4cc2ff" onclick="showfrozen(-1)">冻&nbsp;&nbsp;&nbsp;&nbsp;结</div>
			                          <div class="addbutton" style="background: #4cc2ff" onclick="allunfrozen()">解除冻结</div>
				...
				<table border="0" cellspacing="0" cellpadding="0"
					...
					<s:iterator value="cuserlist" var="cuser">
						...
						<td style="width: 52px;" class="border_right_bottom"><input type="checkbox" name="checkbox" value="${coachid}" ></td>
							...
							<td style="width: 200px;" class="border_noright_bottom">
								...
								<s:if test="isfrozen==0">
								<div class="table_edit_button" style="width: 90px;">
									<div class="table_button_edit_icon"></div>
									<div class="table_button_text" onclick="showfrozen('${coachid}')">冻&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;结</div>
								</div>
								</s:if>
								...
								<div class="table_edit_button" style="width: 90px;">
									<div class="table_button_edit_icon"></div>
									<div class="table_button_text" onclick="unfrozen('${coachid}')">解除冻结</div>
									</div>
								</s:else>
					...
					var a = [];
					  ...
					  if (pageindex == 1) {
					    ...
					    a[a.length] = "<a onclick=\"previousPage("+pageindex+",'getCoachTobeChecked.do?searchname=" + realname+ "&searchphone=" + phone + "&driveSchoolname=" + driveSchoolname+ "&carlicense=" + carlicense + "&index=" + index+"&change_id="+j+"&checkstate="+checkstate+"&')\" class=\"page_prev\"></a>";
					  }
					  function setPageList() {
					    if (pageindex == i) {
					      a[a.length] = "<a onclick=\"goPage('getCoachTobeChecked.do?searchname="+realname+"&searchphone="+phone+"&driveSchoolname="+driveSchoolname+"&carlicense="+carlicense+"&index="+index+"&change_id"+j+"&',"+i+")\" class=\"on\">" + i + "</a>";
					    } else {
					      a[a.length] = "<a onclick=\"goPage('getCoachTobeChecked.do?searchname=" + realname+ "&searchphone=" + phone + "&driveSchoolname=" + driveSchoolname+ "&carlicense=" + carlicense + "&index=" + index+"&change_id="+j+"&checkstate="+checkstate+"&',"+i+")\">" + i + "</a>";
					    }
					  }
					  //总页数小于10
					  if (count <= 10) {
					    ...
					    if (pageindex <= 4) {
					      ...
					      a[a.length] = "...<a onclick=\"goPage('getCoachTobeChecked.do?searchname=" + realname+ "&searchphone=" + phone + "&driveSchoolname=" + driveSchoolname+ "&carlicense=" + carlicense + "&index=" + index+"&change_id="+j+"&checkstate="+checkstate+"&',"+count+")\">" + count + "</a>";
					    } else if (pageindex >= count - 3) {
					      a[a.length] = "<a onclick=\"goPage('getCoachTobeChecked.do?searchname=" + realname+ "&searchphone=" + phone + "&driveSchoolname=" + driveSchoolname+ "&carlicense=" + carlicense + "&index=" + index+"&change_id="+j+"&checkstate="+checkstate+"&',1)\">1</a>...";
					  ...
					  if (pageindex == count) {
						    ...
						    "<a class=\"jump_btn\" onclick=\"gotoPage('getCoachTobeChecked.do?searchname=" + realname+ "&searchphone=" + phone + "&driveSchoolname=" + driveSchoolname+ "&carlicense=" + carlicense + "&index=" + index+"&change_id="+j+"&checkstate="+checkstate+"&',"+$("#pageSize").val()+")\")\">"+
						    "<a id='page_msg'></a>";
						  } else {
						    a[a.length] = 
						    	"<a onclick=\"nextPage("+$("#pageIndex").val()+",'getCoachTobeChecked.do?searchname=" + realname+ "&searchphone=" + phone + "&driveSchoolname=" + driveSchoolname+ "&carlicense=" + carlicense + "&index=" + index+"&change_id="+j+"&checkstate="+checkstate+"&')\" "+
						    	"class=\"page_next\"></a> 共"+count+"页 到第 "+
						    "<input type=\"text\" class=\"jump_num\" id=\"topage\"/> 页"+
						    "<a class=\"jump_btn\" onclick=\"gotoPage('getCoachTobeChecked.do?searchname="+realname+"&searchphone="+phone+"&driveSchoolname="+driveSchoolname+"&carlicense="+carlicense+"&index="+index +"&',"+$("#pageSize").val()+")\">"+
						    "<a id='page_msg'></a>";
						  }
...
<!-- 设置保证金弹框 -->
	...
	<div id="mask_sec" style="position: fixed; width: 100%; height: 300px;z-index: 300">
		...
		<input id="gmoney" type="text" style="width: 170px;height: 40px;margin: auto;margin-left: 70px;margin-top: 45px;font-size: 18px;text-align: center;"  placeholder="请输入保证金金额"/>
...
<!-- 设置保教练等级弹框 -->
	...
	<div id="level_sec" style="position: fixed; width: 100%; height: 300px;z-index: 300;">
		...
		<input id="clevel" type="text" style="width: 170px;height: 40px;margin: auto;margin-left: 70px;margin-top: 45px;font-size: 18px;text-align: center;"  placeholder="设置教练等级"/>
...
<!-- 数据导出弹框 -->
	...
	<div id="alertbox_sec" style="position: fixed; width: 100%; height: 300px;z-index: 300;">
		...
		<br>
		<input type="checkbox" style="margin-top: 5px;" name="C1"  id="a1" value="34"> 教练冻结开始时间
		<input type="checkbox" style="margin-left: 5px;margin-top: 5px;" name="C1"  id="a1" value="35"> 教练冻结结束时间
		<br>
