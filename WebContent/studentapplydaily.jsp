...
function search(){
	...
	window.location.href = "studentapply.do?addtime="+addtime;
}

function dataExport(){
	var addtime = $("#addtime").val();
	if (confirm("确认导出数据？")) {
		window.location.href="studentApplyNoticeExport.do?addtime="+addtime;
	}
}
</script>
<title>学生提现日报</title>
...
<div id="content_form_table">
				<table border="0" cellspacing="0" cellpadding="0"
					...
					<s:iterator value="studentapplylist" status="st">
						<tr class="tr_td">
							...
							<td style="width: 150px;" class="border_right_bottom">${beginaskmoney}</td>
							<td style="width: 100px;" class="border_right_bottom">${nowaskmoney}</td>
							<td style="width: 100px;" class="border_right_bottom">${nowallaskmoney}</td>
							<td style="width: 100px;" class="border_right_bottom">${dmoney}</td>
							<td style="width: 100px;" class="border_right_bottom">${wdmoney}</td>
						</tr>
					</s:iterator>
										<tr>
				...
				var container = container;
					...
					var a = [];
					  ...
					  if (pageindex == 1) {
					    ...
					    a[a.length] = "<a onclick=\"previousPage("+pageindex+",'studentapply.do?addtime="+addtime+"&')\" class=\"page_prev\"></a>";
					  }
					  function setPageList() {
					    if (pageindex == i) {
					      a[a.length] = "<a onclick=\"goPage('studentapply.do?addtime="+addtime+"&',"+i+")\" class=\"on\">" + i + "</a>";
					    } else {
					      a[a.length] = "<a onclick=\"goPage('studentapply.do?addtime="+addtime+"&',"+i+")\">" + i + "</a>";
					    }
					  }
					  //总页数小于10
					  if (count <= 10) {
					    ...
					    if (pageindex <= 4) {
					      ...
					      a[a.length] = "...<a onclick=\"goPage('studentapply.do?addtime="+addtime+"&',"+count+")\">" + count + "</a>";
					    } else if (pageindex >= count - 3) {
					      a[a.length] = "<a onclick=\"goPage('studentapply.do?addtime="+addtime+"&',1)\">1</a>...";
					  ...
					  if (pageindex == count) {
						    ...
						    "<a class=\"jump_btn\" onclick=\"gotoPage('studentapply.do?addtime="+addtime+"&',"+$("#pageSize").val()+")\")\">"+
						    "<a id='page_msg'></a>";
						  } else {
						    a[a.length] = 
						    	"<a onclick=\"nextPage("+$("#pageIndex").val()+",'studentapply.do?addtime="+addtime+"&')\" "+
